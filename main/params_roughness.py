...
k = lambda x, dm: (1 - isFractureDomain(dm)) * Identity(dim) + isFractureDomain(
    dm
) * kf * Identity(dim)
...
gn = lambda x, dm: 0.0
boundary_dn = lambda x, dm: conditional(x[0] < 1e-12, 1, 0) + conditional(
    x[0] > 1.0 - 1e-12, 1, 0
)
gn_gamma = lambda x: 0.0
boundary_dn_gamma = lambda x: 0.0
q = lambda x, dm: 200 * (
    tanh(200 * (0.025 - sqrt(dot(x, x))))
    - tanh(200 * (0.025 - sqrt((x[0] - 1) ** 2 + (x[1] - 1) ** 2)))
)
q_gamma = lambda x: 0.0
