...
if roughness:  # fracture for data from "roughness series"
    ...
    d1 = lambda x, d0: d0 + 0.5 * d_ampl(d0) * (
        np.sin(8 * np.pi * x[1]) + np.sin(8 * np.pi * x[2])
    )

    if symm:
        d2 = lambda x, d0: d0 + 0.5 * d_ampl(d0) * (
            np.sin(8 * np.pi * x[1]) + np.sin(8 * np.pi * x[2])
        )
    else:
        d2 = lambda x, d0: d0 - 0.5 * d_ampl(d0) * (
    ...
    fileseries = (
        "comparison_"
        + ("tang" if tangential else "perp")
        + "_"
        + ("sym" if symm else "asym")
        + f"_dim{dim}"
    )
...
def get_point_data(reader_data, value):
    ...
    if point_scalar_idx < 0:
        ...
        point_scalar_vtk_array = reader_data.GetPointData().GetArray(point_scalar_idx)
...
for k in range(len(d0_values)):
    ...
    if roughness:
        vtp_files += [
            filename + "_mmdg2_dmean_trafoGamma0",
    ...
    if dim == 2:
        ...
        im_p = ax_p.tricontourf(xi.flat, yi.flat, p_grid.flat, cmap=cmap, levels=51)
        fig_p.colorbar(im_p)
        ...
        fig_p.savefig(
            join(output_dir, filename + "_fulldim_pressure.pdf"), bbox_inches="tight"
        )
        fig_u.savefig(
            join(output_dir, filename + "_fulldim_velocity.pdf"), bbox_inches="tight"
        )
        plt.close(fig_p)
        plt.close(fig_u)
        ...
        for i in range(len(vtp_files)):
            vtp_reader = XMLPolyDataReader(
                FileName=join(vtk_dir, vtp_files[i] + ".vtp")
            )
            ...
            SaveData(
                csv_file, plot_over_line, Precision=15, FieldAssociation="Point Data"
            )
            ...
            ax_err.plot(y, p - p_gamma, linestyle=line_styles[i], color=colors[i])
        ...
        fig_sol.savefig(
            join(output_dir, filename + "_pressure.pdf"), bbox_inches="tight"
        )

        ax_err.set_xlabel(r"$x_2$")
        ax_err.set_ylabel(r"error  $p_\Gamma^\mathrm{ref.}(x_2 ) - p_\Gamma (x_2 )$")
        fig_err.savefig(join(output_dir, filename + "_error.pdf"), bbox_inches="tight")
    ...
    ############################################################################
        ...
        for i in range(len(vtp_files)):
            ...
            error[i, k] = np.sqrt((h**2) * np.nansum(np.square(p - p_gamma).flat))
        ...
        im_p_gamma = ax_p_gamma.tricontourf(
            ydata.flat,
            zdata.flat,
            p.flat,
            cmap="viridis",
        ...
        fig_p_gamma.savefig(
            join(output_dir, filename + "_reference_p_gamma.pdf"), bbox_inches="tight"
        )
        ...
        for i in range(len(vtp_files)):
            ...
            ax_p_gamma.set_ylabel(r"$x_3$")
            fig_p_gamma.savefig(
                join(output_dir, vtp_files[i] + ".pdf"), bbox_inches="tight"
            )
            plt.close(fig_p_gamma)
...
if len(files) > 1:
    ...
    fig_l2.savefig(join(output_dir, fileseries + "_L2.pdf"), bbox_inches="tight")
    plt.close(fig_l2)
