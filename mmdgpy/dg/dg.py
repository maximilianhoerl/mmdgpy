...
class DG:
    ...
    def __init__(
        ...
        mu = (
            Constant(mu0, name="mu0")
            * (order + 1)
            * (order + dim)
            / MaxFacetEdgeLength(self.space.cell())
        )

        self.iterms = (
            mu * jump(self.p) * jump(self.phi)
            - jump(self.phi)
            * dot(avg(self.problem.k(self.x, self.dm) * grad(self.p)), self.n("+"))
            - jump(self.p)
            * dot(avg(self.problem.k(self.x, self.dm) * grad(self.phi)), self.n("+"))
        )

        self.b_bulk = (
            dot(self.problem.k(self.x, self.dm) * grad(self.p), grad(self.phi)) * dx
        )
        ...
        self.b_bulk -= (
            ...
            * self.problem.boundary_dn(self.x, self.dm)
            * ds
        )
        self.b_bulk -= (
            self.phi
            * dot(self.problem.k(self.x, self.dm) * grad(self.p), self.n)
            * self.problem.boundary_dn(self.x, self.dm)
            * ds
        )
        ...
        if self.problem.gd(self.x, self.dm) != 0:
            self.l_bulk += (
                mu
                * self.problem.gd(self.x, self.dm)
                * self.phi
                * self.problem.boundary_dn(self.x, self.dm)
                * ds
            )
        ...
        if (
            self.problem.boundary_dn(self.x, self.dm) != 1
            and self.problem.gn(self.x, self.dm) != 0
        ):
            self.l_bulk += (
                self.problem.gn(self.x, self.dm)
                * self.phi
                * (1.0 - self.problem.boundary_dn(self.x, self.dm))
                * ds
            )
    ...
    def write_vtk(self, filename="pressure", filenumber=0, detailed=False):
        ...
        self.omega.writeVTK(
            ...
            pointdata=pointdata,
            nonconforming=True,
            subsampling=self.space.order - 1,
        )
    ...
    def get_error(self, order):
        ...
        return sqrt(
            integrate(
                self.omega,
                (self.ph - self.problem.p(self.x, self.dm)) ** 2,
                order=order,
            )
        )
