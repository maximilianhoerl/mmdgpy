...
class MMDG2(DG):
    ...
    def __init__(
        ...
        mu_gamma = (
            Constant(mu0, name="mu0_gamma")
            * (order + 1)
            * (order + dim - 1)
            / MaxFacetEdgeLength(self.ispace.cell())
        )
        ...
        self.b_gamma = (
            dot(
                ...
                * grad(self.problem.d(self.x_gamma) * p_gamma),
                grad(self.phi_gamma),
            )
            * dx
        )
        ...
        self.b_gamma -= (
            jump(self.problem.d(self.x_gamma) * p_gamma)
            * dot(
                avg(self.problem.k_gamma(self.x_gamma) * grad(self.phi_gamma)),
                self.n_gamma("+"),
            )
            * dS
        )
        ...
        if self.problem.boundary_dn_gamma(self.x_gamma) != 0:
            ...
            self.b_gamma -= (
                ...
                * p_gamma
                * dot(
                    self.problem.k_gamma(self.x_gamma) * grad(self.phi_gamma),
                    self.n_gamma,
                )
                * self.problem.boundary_dn_gamma(self.x_gamma)
                * ds
        ...
        if self.problem.q_gamma(self.x_gamma) != 0:
            self.l_gamma += self.problem.q_gamma(self.x_gamma) * self.phi_gamma * dx

        if (
            self.problem.gd_gamma(self.x_gamma) != 0
            and self.problem.boundary_dn_gamma(self.x_gamma) != 0
        ):
            self.l_gamma += (
                mu_gamma
                * self.problem.gd_gamma(self.x_gamma)
                * self.phi_gamma
                * self.problem.boundary_dn_gamma(self.x_gamma)
                * ds
            )
        ...
        if (
            self.problem.boundary_dn_gamma(self.x_gamma) != 1
            and self.problem.gn_gamma(self.x_gamma) != 0
        ):
            self.l_gamma += (
                self.problem.gn_gamma(self.x_gamma)
                * self.phi_gamma
                * (1.0 - self.problem.boundary_dn_gamma(self.x_gamma))
                * ds
            )

        alpha = problem.k_gamma_perp(self.x) / self.problem.d(self.x)("+")
        alpha_gamma = self.problem.k_gamma_perp(self.x_gamma) / self.problem.d(
            self.x_gamma
        )
        beta = 4 * alpha / (2 * xi - 1)
        ...
        if contortion:
            ...
            self.c_bulk = (
                ...
                * (
                ...
                / sqrt(1.0 + dot(grad_d_minus, grad_d_minus))
                * dS
            )
            self.c_bulk += (
                self.chi_gamma
                ...
                * (
                ...
                / sqrt(1.0 + dot(grad_d_plus, grad_d_plus))
                * dS
            )

        else:
            self.c_bulk = alpha * jump(self.p) * jump(self.phi) * self.chi_gamma * dS
            self.c_bulk += (
                beta
                * (avg(self.p) - skeleton(self.ph_gamma, grid=self.omega)("+"))
                * avg(self.phi)
        ...
        self.c_gamma = (
            beta_gamma
            * (p_gamma - avg(trace(self.ph, self.igridview)))
            * self.phi_gamma
            * dx
        )
    ...
    def solve(
        ...
        """
        scheme = galerkin(
            [self.b_bulk + self.c_bulk == self.l_bulk],
            solver=("suitesparse", "umfpack"),
        )
        scheme_gamma = galerkin(
            [self.b_gamma + self.c_gamma == self.l_gamma],
            solver=("suitesparse", "umfpack"),
        )

        if solver == "monolithic":
            monolithicSolve(
                schemes=(scheme, scheme_gamma),
                targets=(self.ph, self.ph_gamma),
                iter=iter,
                tol=tol,
                f_tol=f_tol,
                eps=eps,
    ...
    def get_error(self, order):
        ...
        error_gamma = integrate(
            self.igridview,
            (self.ph_gamma - self.problem.p_gamma(self.x_gamma)) ** 2,
            order=order,
        )
