from ufl import cosh, exp, Identity, sinh, cos, sin, sqrt, tanh
...
class MMDGProblem2(MMDGProblemVertical):
    ...
    :ivar dmax: The maximum aperture of the fracture.
    """

    def __init__(self, dmax):
        """The constructor.

        :param float dmax: The maximum aperture of the fracture.
        """
    ...
    def q_gamma(self, x):
        ...
        :param x: The spatial coordinate.
        """
        return -32 * self.d(x) * sin(4 * x[1]) - 2 * sqrt(2) * sinh(
            2 * sqrt(2)
        ) * self.p_gamma(x)
