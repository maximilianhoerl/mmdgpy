...
class ProblemRoughness(ProblemComparison):
    ...
    representation so that we provide methods to interpolate them on a the given
    grid.
    """
    # Attributes:
    ## @var _q
    ...
    ## @var _bulk_d1
    # The aperture d_1 of the fracture left to the interface in bulk
    # coordinates.
    ## @var _bulk_d2
    # The aperture d_2 of the fracture right to the interface in bulk
    # coordinates.
    ## @var _i_d1
    # The aperture d_1 of the fracture left to the interface in interface
    # coordinates.
    ## @var _i_d2
    # The aperture d_2 of the fracture right to the interface in interface
    # coordinates.
    ## @var _bulk_grad_d1
    # The gradient of the aperture function d1 in bulk coordinates.
    ## @var _bulk_grad_d2
    # The gradient of the aperture function d2 in bulk coordinates.
    ## @var _i_grad_d1
    # The gradient of the aperture function d1 in interface coordinates.
    ## @var _i_grad_d2
    # The gradient of the aperture function d2 in interface coordinates.
    ## @var _grid
    # The grid view of a dG method.

    def __init__(self, gd, gn, boundary_dn, k, d1, d2, gd_gamma, gn_gamma, \
     boundary_dn_gamma, k_gamma, k_gamma_perp, q, q_gamma):
        """! The constructor.
            ...
            @param boundary_dn  Indicator function to distinguish between
                                Dirichlet (1) and Neumann (0) conditions on the
                                boundary of the bulk domain.
            ...
            @param boundary_dn_gamma  Indicator function to distinguish between
                                      Dirichlet (1) and Neumann (0) conditions
        ...
        """
        super().__init__(gd, gn, boundary_dn, k, d1, d2, gd_gamma, gn_gamma, \
         boundary_dn_gamma, k_gamma, k_gamma_perp)
        self._q = q
    ...
    def initialize(self, grid):
        ...
        ivectorspace = finiteVolume(grid.hierarchicalGrid.interfaceGrid,
        ...
        self._bulk_grad_d1 = \
         bulkvectorspace.interpolate(grad(self._bulk_d1), name="bulk_grad_d1")
        self._bulk_grad_d2 = \
         bulkvectorspace.interpolate(grad(self._bulk_d2), name="bulk_grad_d2")
        self._i_grad_d1 = \
         ivectorspace.interpolate(grad(self._i_d1), name="i_grad_d1")
        self._i_grad_d2 = \
         ivectorspace.interpolate(grad(self._i_d2), name="i_grad_d2")
    ...
    def _interpolate(self, fct_np, fct_ufl):
        ...
        for v in grid.vertices:
            ...
            fct_ufl.as_numpy[:][idx] = fct_np([cnt[0], cnt[1], 0.])
