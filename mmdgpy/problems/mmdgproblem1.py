...
class MMDGProblem1(MMDGProblemVertical):
    ...
    # Attributes:
    ## @var d0
    # The constant aperture of the fracture.

    def __init__(self, d0):
        """! The constructor.

            @param d0  The constant aperture of the fracture.
        """
    ...
    def p(self, x, dm):
        ...
        """
        return (1 - dm) * sin(4 * x[0]) * cos(pi * x[1]) \
         + dm * cos(4 * x[0]) * cos(pi * x[1])
    ...
    def p_gamma(self, x):
        """! The exact solution p_gamma inside the reduced fracture.
            ...
            @param x  The spatial coordinate.
        """
        return 0.75 * (cos(2.) + sin(2.)) * cos(pi * x[1])
    ...
    def q_gamma(self, x):
        """! The source term q_gamma inside the reduced fracture.
            ...
            @param x  The spatial coordinate.
        """
        return (0.75 * self.d0 * pi**2 + 4) * (cos(2.) + sin(2.)) \
         * cos(pi * x[1])
    ...
    def gn_gamma(self, x):
        """! The velocity at the boundary of the reduced fracture (Neumann
            ...
            @param x  The spatial coordinate.
        """
        return -0.75 * pi * (cos(2.) + sin(2.)) * sin(pi * x[1])
