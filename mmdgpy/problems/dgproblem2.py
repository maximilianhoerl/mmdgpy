...
class DGProblem2(DGProblem):
    ...
    # Attributes:
    ## @var d_mean
    # The mean aperture of the fracture.
    ## @var d_diff
    # The maximum aperture fluctuation of the fracture.

    def __init__(self, d_mean, d_diff):
        """! The constructor.

            @param d_mean  The mean aperture of the fracture.
            @param d_diff  The maximum aperture fluctuation of the fracture.
        """
    ...
    def p(self, x, dm):
        ...
        return conditional(x[0] < 0.5 * ( 1 - self.d(x) ), p1, \
    ...
    def q(self, x, dm):
        """! The source term q.
            ...
            @param dm  A domain marker.
        """
        q1 = -2. / ( 1 - self.d(x) ) * exp(-self.d(x)) \
         * ( ( 1 - x[0] - self.d(x) ) * ( self._d_prime_prime(x) \
         + (self._d_prime(x) ** 2) * self.d(x) / ( 1 - self.d(x) ) ) \
         - self._d_prime(x) ** 2 )
        q2 = -2. * x[0] / ( 1 + self.d(x) ) * exp(self.d(x)) \
         * ( self._d_prime_prime(x) + (self._d_prime(x) ** 2) * self.d(x) \
         / ( 1 + self.d(x) ) )
        qf = -1 * ( 4 + self._d_prime_prime(x) ) * exp(2 * x[0] - 1)

        return conditional(x[0] < 0.5 * ( 1 - self.d(x) ), q1, \
    ...
    def k(self, x, dm):
        ...
        for i in range(dim):
            ...
            row1[i] = 1. / ( 1 - self.d(x) ) if i > 0 else 1
            row2[i] = 1. / ( 1 + self.d(x) ) if i > 0 else 1
            rowf[i] = 1. / ( 1 + self.d(x) ) if i == 0 else 1
            k1 += [row1]
            k2 += [row2]
            kf += [rowf]

        return conditional(x[0] < 0.5 * ( 1 - self.d(x) ), as_matrix(k1), \
         conditional(x[0] > 0.5 * ( 1 + self.d(x) ), as_matrix(k2), \
         as_matrix(kf)))
