...
def create_resolved_grid(file, d1, d2, h, hf, dim):
    ...
    hf = 1.0 / np.ceil(1.0 / hf)
    pos = np.arange(0.0, 1.0 + 1e-12, hf)
    N = len(pos)
    ...
    if dim == 2:
        ...
        for i in range(N):
            points_gamma1.append(
                geo.addPoint(0.5 - d1([0.5, pos[i], 0]), pos[i], 0, hf, 100 + 2 * i)
            )
            points_gamma2.append(
        ...
        for i in range(N - 1):
            lines_gamma1.append(
                geo.addLine(points_gamma1[i], points_gamma1[i + 1], 100 + 2 * i)
            )
            lines_gamma2.append(
                geo.addLine(
        ...
        for i in range(N - 1):
            for j in range(N):
                lines_gamma1_x2[i, j] = geo.addLine(
                    points_gamma1[i, j], points_gamma1[i + 1, j]
                )
                lines_gamma2_x2[i, j] = geo.addLine(
                    points_gamma2[i, j], points_gamma2[i + 1, j]
                )

        for i in range(N):
            for j in range(N - 1):
                lines_gamma1_x3[i, j] = geo.addLine(
                    points_gamma1[i, j], points_gamma1[i, j + 1]
                )
                lines_gamma2_x3[i, j] = geo.addLine(
                    points_gamma2[i, j], points_gamma2[i, j + 1]
                )
        ...
        cf_1234 = geo.addCurveLoop(
            np.concatenate(
                (
                    lines_gamma1_x2[:, 0],
                    [lf_34],
                    -np.flip(lines_gamma2_x2[:, 0]),
                    [-lf_12],
                )
            )
        )
        cf_1256 = geo.addCurveLoop(
            np.concatenate(
                (
                    lines_gamma1_x3[0, :],
                    [lf_56],
                    -np.flip(lines_gamma2_x3[0, :]),
                    [-lf_12],
                )
            )
        )
        cf_3478 = geo.addCurveLoop(
            np.concatenate(
                (
                    lines_gamma1_x3[N - 1, :],
                    [lf_78],
                    -np.flip(lines_gamma2_x3[N - 1, :]),
                    [-lf_34],
                )
            )
        )
        cf_5678 = geo.addCurveLoop(
            np.concatenate(
                (
                    lines_gamma1_x2[:, N - 1],
                    [lf_78],
                    -np.flip(lines_gamma2_x2[:, N - 1]),
                    [-lf_56],
                )
            )
        )
        ...
        for i in range(N - 1):
            for j in range(N - 1):
                curves_gamma1[i, j] = geo.addCurveLoop(
                    [
                        lines_gamma1_x2[i, j],
                        lines_gamma1_x3[i + 1, j],
                        -lines_gamma1_x2[i, j + 1],
                        -lines_gamma1_x3[i, j],
                    ]
                )
                curves_gamma2[i, j] = geo.addCurveLoop(
                    [
                        lines_gamma2_x2[i, j],
                        lines_gamma2_x3[i + 1, j],
                        -lines_gamma2_x2[i, j + 1],
                        -lines_gamma2_x3[i, j],
                    ]
                )
        ...
        slf = geo.addSurfaceLoop(
            np.concatenate(
                (
                    [sf_1234, sf_1256, sf_3478, sf_5678],
                    surf_gamma1.flat,
                    surf_gamma2.flat,
                )
            )
